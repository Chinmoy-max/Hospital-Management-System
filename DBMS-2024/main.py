...
def doctors():
    if request.method=="POST":
        ...
        newdoc=Doctors(email=email,doctorname=doctorname,dept=dept)        
...
# mail starts from here
        ...
        newpatient=Patients(email=email,name=name,gender=gender,slot=slot,disease=disease,time=time,date=date,dept=dept,number=number)        
...
def bookings():
    ...
    with db.engine.connect() as conn:
        query=conn.execute(f"SELECT * FROM `patients` WHERE email='{em}'")
